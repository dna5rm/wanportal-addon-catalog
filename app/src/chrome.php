<?php
/**
 * chrome.php: shared page chrome for the wanportal-addon-catalog sidecar.
 *
 * Used by every sidecar page:
 *   - catalog listing  /catalog/index.php
 *   - SKU detail/edit  /catalog/sku.php
 *   - lookup admin     /catalog/admin.php
 *
...
if (defined('WANPORTAL_ADDON_CHROME_LOADED')) {
    return;
}
define('WANPORTAL_ADDON_CHROME_LOADED', '1');
 ...
 */
if (!defined('WANPORTAL_ADDON_CHROME_EMBED')) {
    define('WANPORTAL_ADDON_CHROME_EMBED', isset($_GET['embed']) || !empty($_SERVER['HTTP_X_WANPORTAL_EMBED']));
}
...
JS;
     ...
     * (home|catalog|admin) or an exact href.
     *
    ...
    function nb_chrome_foot(string $extraHtml = ''): void {
        echo "<footer class=\"addon-foot\">wanportal &middot; catalog sidecar</footer>\n";
        if ($extraHtml !== '') {
            echo $extraHtml, "\n";
        }
        echo "</body>\n</html>\n";
...
.addon-foot a { color: var(--muted); }
.addon-foot a:hover { color: var(--text); }
 ...
 * Bootstrap loads before the chrome style and its variant classes win
 * the cascade, so every shape used in sidecar markup (plain, sm, info,
 * primary, outline-*, on <button> or <a>) maps through the selectors
 * here to the same face as ui/src/styles/base.css; danger keeps a
 * --danger border so destructive controls still read. */
.btn,
...
/* Logged-in only (Admin nav, edit controls). Hidden until body.authed. */
.needs-auth { display: none !important; }
body.authed a.nav-link.needs-auth { display: inline !important; }
body.authed .needs-auth { display: revert !important; }
body.authed td.needs-auth, body.authed th.needs-auth { display: table-cell !important; }
CSS;
    }
